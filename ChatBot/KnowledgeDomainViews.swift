...
struct DomainSettingsContent: View {
    ...
    var body: some View {
                            ...
                            HStack {
                                VStack(alignment: .leading, spacing: 2) {
                                    ...
                                    HStack(spacing: 4) {
                                        ...
                                        let wikiCount = store.wikiStore.pages(forDomain: domain.id).count
                                        Text("\(wikiCount) wiki page\(wikiCount == 1 ? "" : "s")")
                                        Text("\u{00B7}")
        ...
        .alert("Delete Domain?", isPresented: Binding(
            ...
            Text("All knowledge bases in this domain will be moved to General.")
        }
    }
}
...
struct DomainDetailContent: View {
    ...
    @State private var showingAddWikiPage = false
    @State private var showingKBPicker = false
    @State private var editingWikiPage: WikiPage?
    ...
    private var domainWikiPages: [WikiPage] {
        store.wikiStore.pages(forDomain: domain.id)
    }
    ...
    var body: some View {
        List {
            ...
            // MARK: Wiki Pages Section
            Section {
                if domainWikiPages.isEmpty {
                    Text("No wiki pages in this domain.")
                        .foregroundStyle(.secondary)
                        .font(.subheadline)
                } else {
                    ForEach(domainWikiPages) { page in
                        Button {
                            editingWikiPage = page
                        } label: {
                        ...
                        .contextMenu {
                            ...
                            Button(role: .destructive) {
                                Task { await store.wikiStore.deletePage(id: page.id) }
                            } label: {
                                Label("Delete", systemImage: "trash")
                            }
                        }
        ...
        .sheet(isPresented: $showingAddWikiPage) {
            WikiPageEditorView(wikiStore: store.wikiStore, domainID: domain.id)
        }
        .sheet(item: $editingWikiPage) { page in
            WikiPageEditorView(wikiStore: store.wikiStore, existing: page)
        }
