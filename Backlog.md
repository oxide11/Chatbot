...
- [ ] **OpenAI provider** — modelled on `AnthropicProvider`. Chat Completions API + SSE streaming. Per-task routing already supports it; just needs the implementation in `ProviderRegistry.resolve(_:)`.
- [ ] **Gemini provider** — same shape, different wire format (Server-Sent Events on `/v1beta/models/...:streamGenerateContent`).
...
- [ ] **Schema-based extraction (`@Generable`).** The wiki extraction and lint pipelines currently hand-parse a `---PAGE---/---END---` text block format. Apple's `FoundationModels` framework supports `@Generable` types where the model returns a typed Swift struct directly — strict, no parser, fewer "looks-right-but-doesn't-parse" failures. Migrate `ExtractionResult` and the lint merge / stub outputs to `@Generable`. Anthropic / OpenAI / Gemini paths would still use the text format (or each provider's structured-output mode) — the abstraction lives in `WikiEngine.respondOnce`.
