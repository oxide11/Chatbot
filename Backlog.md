...
- [ ] **Prompt audit and optimisation.** Every system prompt and worker prompt could be tightened: the wiki extraction prompts (conversation + document modes), the lint review prompts (merge / stub), the chat default instructions, the per-worker prompts in `BuiltInWorkers`. Output schemas could also move from the loose `---PAGE---/---END---` format to `GenerationSchema` for stricter parsing.
